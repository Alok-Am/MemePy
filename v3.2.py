...
def stitch_videos(video_clips, output_path):
    try:
        ...
        if video_clips:
            ...
            # Move processed clips to the "processed" folder
            for clip in video_clips:
                clip_path = Path(clip.filename).as_posix()
                new_clip_name = f"{os.path.splitext(os.path.basename(clip_path))[0]}_{os.path.splitext(os.path.basename(output_path))[0]}.mp4"
                processed_clip_path = os.path.join(PROCESSED_FOLDER, new_clip_name)
